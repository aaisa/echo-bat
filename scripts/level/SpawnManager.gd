...
func _spawn_pair(x: float, params: Dictionary) -> void:
	...
	if top_h > 1.0:
		_add_stalactite(pair, Vector2(0.0, top_h * 0.5), Vector2(OBSTACLE_WIDTH, top_h))

	# Estalactita inferior: desde cy + gap_half hasta el suelo (y=660)
	var bot_top: float = cy + gap_half
	var bot_h: float = FLOOR_Y - bot_top
	if bot_h > 1.0:
		_add_stalactite(pair, Vector2(0.0, (bot_top + FLOOR_Y) * 0.5), Vector2(OBSTACLE_WIDTH, bot_h))

	_active_pairs.append(pair)

func _add_stalactite(parent: Node2D, local_pos: Vector2, size: Vector2) -> void:
